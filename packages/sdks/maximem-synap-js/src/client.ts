...
export interface SynapClientOptions {
   ...
   */
  st_verbatim_overlay?: boolean;
  stVerbatimOverlay?: boolean;
   ...
   */
  sdk_st_authoritative?: boolean;
  sdkStAuthoritative?: boolean;
...
export class SynapClient {
  ...
  constructor(options: SynapClientOptions = {}) {
    ...
    this.conversation = createConversationNamespace(
      ...
      (response, conversationId, userId) => {
        ...
        this.#injectUserSummary(response, conversationId, userId);
      },
      {
  ...
  #injectUserSummary(
    ...
    const summary = this.#anticipationCache.lookupUserSummary(userId);
