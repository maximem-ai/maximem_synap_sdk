...
from maximem_synap.models.context import Fact, ResponseMetadata, UnifiedContextResponse
...
def _resp(**kw):
    d = {"facts": [Fact(id="f1", content="User is an engineer", confidence=0.9, source="test", extracted_at=datetime.now(timezone.utc))],
         "preferences": [], "scope_map": {"f1": "user"}, "scopes_queried": ["user"], "total_items": 1,
...
def mock_sdk():
    ...
    r = MagicMock(); r.ingestion_id = "ing-001"
